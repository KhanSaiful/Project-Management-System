...
def send_notify_mail(new_task):
    ...
    if not new_task.assigned_to == new_task.created_by:
        ...
        email_body = render_to_string(
            "todo/email/assigned_body.txt", {"task": new_task,
                                             "site": current_site}
        )
...
def send_email_to_thread_participants(task, msg_body, user, subject=None):
    ...
    current_site = Site.objects.get_current()
    email_subject = (
        subject if subject else render_to_string(
            "todo/email/assigned_subject.txt", {"task": task})
    )
    email_body = render_to_string(
    ...
    send_mail(email_subject, email_body, task.created_by.email,
              recip_list, fail_silently=False)
